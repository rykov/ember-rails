...
module.exports = {
  ...
  embroiderBuild(app, options) {
    ...
    const { postprocessAppTree, stripSourcemaps } = opts;
    delete opts['postprocessAppTree'];
    delete opts['stripSourcemaps'];
    if (typeof stripSourcemaps === 'string' && stripSourcemaps !== 'hidden') {
      const str = JSON.stringify(stripSourcemaps);
      throw new SilentError(`Invalid stripSourcemaps value ${str}; expected true, false, or "hidden".`);
    }
    ...
    if (stripSourcemaps) {
      const StripSourcemaps = require('./strip-sourcemaps');
      tree = new StripSourcemaps([tree], { keepMaps: stripSourcemaps === 'hidden' });
    }
    return tree;
  },
