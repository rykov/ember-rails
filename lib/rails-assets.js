...
function AssetPackager(inTree, options) {
  ...
  // Add all asset paths to project
  var assetPaths = this.project.assetPaths = [];
  for(var src in options.packagePaths) {
    var dst = options.packagePaths[src];
    dst = path.relative("/assets", dst);
    assetPaths.push(dst);
  }
};
